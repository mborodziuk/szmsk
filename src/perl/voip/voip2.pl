...
while (@row = $sth->fetchrow_array() )
  {
		...
		$url="http://isas-api.artcom.pl/voip_billing/voip_billing.cgi?phone_no=$numer&month=$m&year=$Y";
 
		$wynik=system("/usr/bin/wget --user=$USER --password='$PASS' '$url' -O $file");
		if (! $wynik)
			{			
				$q2="delete from polaczenia_voip where ( data >= '$Y-$m-02 00:00:00' and data < '$mn 00:00:00')";
				print "$q2 \n\n";
				$sth2=$dbh->prepare($q2);
				$sth2->execute();
				open(UCHWYT, "$file");
