...
fprintf(fpup, "tc filter add dev %s protocol ip parent 1:0 prio 4 u32 ht %s match ip src $GALILEUSZ match ip dst %s flowid %d:%d     && \n",    ifin, ht, ip, classidup, *i);
fprintf(fpup, "tc filter add dev %s protocol ip parent 1:0 prio 4 u32 ht %s match ip src $ZVIDNET match ip dst %s flowid %d:%d  && \n",    ifin, ht, ip, classidup, *i);
fprintf(fpup, "tc filter add dev %s protocol ip parent 1:0 prio 4 u32 ht %s match ip src $FUSION match ip dst %s flowid %d:%d && \n",    ifin, ht, ip, classidup, *i);
*/
...
void dhcp_head2(int b2, FILE *fp, int b1)
{
	...
	if ( b1 == 10 )
	...
	else if ( b1 == BAJT_CORE ) 
		{
		    if ( b2 == 16 )
			{
			fprintf(fp, "subnet 172.16.0.0 netmask 255.255.224.0              \n");
			fprintf(fp, "{                                                 \n");
			fprintf(fp, "deny unknown-clients;                             \n");
			fprintf(fp, "default-lease-time           %d;                  \n", D_LEASE_TIME   );
			fprintf(fp, "max-lease-time               %d;                  \n", MAX_LEASE_TIME );
			...
			fprintf(fp, "option routers               172.16.0.1;                  \n" );
			fprintf(fp, "option ntp-servers  %s;           								       \n", DNS1 );
			}
		    else if ( b2 == 17 )
		    {
									...
									if ( strcmp(arg, "all") ==0 ) 
										{
												...
												if ( ip.b2 != oldbajt )
													    {
														if (  (ip.b1 == 10 && (ip.b2 == 8  ||  ip.b2 == 17 )) ||  (ip.b1 == BAJT_CORE && ( ip.b2 == 16 || ip.b2 == 17 ||  ip.b2 == 30))  || ip.b1 ==  BAJT_PUBL )
															{
																oldbajt=ip.b2;
																fprintf ( fps[32], "} \n"  );
