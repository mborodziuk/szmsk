...
class CUSTOMERS
{
	...
	function AddressAdd($dbh, $id_abon, $wazne_od=Null)
	{
		...
		else if ( $_POST_[a_ulica] != $conf[select] )
			{					    
	...
	function UpdateAbon($dbh, $id_abon, &$plg)
	{
		include "func/config.php";
...
function ListaAbonentow($dbh, $p)
	...
	function ZastosujAbon($dbh, &$windykacja, &$plg, $array1, $array2, $typ='', $array3='')
	{
		include "func/config.php";
			...
			if ( $k!="przycisk" )
				{
						...
						if ($t[0]=="K")
						{
							foreach ($array2 as $k2 => $v2)
								if ( !empty($k2) )
									{ 
										$q2="delete from $k2 where $v2 in (select id_kontakt from kontakty where id_podm='$t[1]')";
										WyswietlSql($q2);	
										Query($dbh, $q2);
							...
							foreach ($array1 as $k1 => $v1)
								if ( !empty($k1) )
								{ 
									$q3="delete from $k1 where $v1='$t[1]'";
									WyswietlSql($q3);	
									Query($dbh, $q3);
								}
							...
							foreach ($array3 as $k3 => $v3)
								{ 
									$q4="delete from $k3 where $v3='$t[1]'";
									WyswietlSql($q4);	
									Query($dbh, $q4);
								}
						}
	...
	*/
	...
	{
			{
				$q1="(select nr_um, data_zaw, data_zycie, typ_um, status from umowy_abonenckie 
						 where id_abon='$id_abon'  order by data_zycie)
						 union
						 (select nr_um, data_zaw, data_zycie, typ_um, status from umowy_serwisowe 
						 where id_abon='$id_abon' order by data_zycie)
						 order by 1";
				...
				while ( $row =$sth1->fetchRow() )
					{
						echo "<tr>";
						echo "<tr><td> $lp.</td>";
						echo "<td colspan=\"1\"> <a href=\"index.php?panel=inst&menu=updateum&um=$row[0]\">$row[0]</a> </td>";
						echo "<td> $row[1] </td>";
						echo "<td> $row[2] </td>";
						echo "<td> $row[3] </td>";
						echo "<td> $row[4] </td>";
		...
		if ( !empty($_POST["nr_um1"]) && !empty($_POST["nr_um2"]))
			{
				...
				'id_prac' => 	FindId2($_POST[id_prac]), 	'data_zycie' 	=> $data_zycie , 'szablon' => $_POST[szablon]
				 );	
	...
	function UpdateNrUm($nr)
{
	...
	function ContractAdd($dbh, $id_abon)
	{
			...
			{
				$nr1=$this->UpdateNrUm($_POST["nr_uma1"]);
				$nr_um="$nr1"."/".$_POST["rodzaj"]."/".$_POST["nr_uma2"];
			}
