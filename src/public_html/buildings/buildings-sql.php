...
function BuildingList($dbh, $p, $www=NULL)
{
	...
	*/
	...
	left join 
	(( inst_vlanu v left join adresy_ip i on v.id_ivn=i.id_urz) left join podsieci p on p.id_pds=i.id_pds)
	on b.id_ivn1=v.id_ivn order by b.id_bud";
...
*/		
   ...
   while ($row =$sth->fetchRow())
     ...
     echo "</tr>";*/
		 $bud=explode("d", $row[0]);
		 $q5.="update miejsca_instalacji set id_bud='bud0$bud[1]' where id_bud='$row[0]';";
		 $q6.="update adresy_siedzib set id_bud='bud0$bud[1]' where id_bud='$row[0]';";
		 $q7.="update budynki set id_bud='bud0$bud[1]' where id_bud='$row[0]';";
		}
		 $q5.="<br>";
		 $q6.="<br>";
		 $q7.="<br>";
...
{
	include "func/config.php";
	$id_bud=IncValue($dbh, $QA15, "bud00");
			...
			if ( ValidateKod($_POST[kod]) )
				{
					...
					$bud=array ('id_bud'	=>$id_bud, 	'id_ul'	=>	$id_ul, 'id_adm'	=>FindId2( $_POST[administracja] ),   'numer'=>$_POST[nr_bud],
								'il_mieszk'	=>IsNull($_POST[il_mieszk]), 	'przylacze'		=>$_POST[przylacze]);
				}
		}		
