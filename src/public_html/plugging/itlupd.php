<?php
	...
	$Q="select data_zgl, id_abon, opis, trudnosc, data_zak from instalacje where id_itl='$_GET[itl]'";			
	WyswietlSql($Q);
	$row=Query2($Q, $dbh);
	
	$_SESSION[$_GET[itl].$_SESSION[login]]=$itla=array(
	'id_itl' 			=> $_GET[itl], 	  'data_zgl'		  => $row[0],    'id_abon'	=> $row[1], 'opis'			=> 	$row[2],
	'trudnosc' => $row[3], 'data_zak' => $row[4]
	);	
...
-->			 		
      ...
      <td>
		<input type="text" name="data_zgl" size="10" value="<?php echo "$itla[data_zgl]";?>"/>
		</td>
    </tr>
		<tr>
      <td> Data zakończenia </td>
      <td>
		<input type="text" name="data_zak" size="10" value="<?php echo "$itla[data_zak]";?>"/>
		</td>
    </tr>
	<tr>
		<td class="klasa2">
