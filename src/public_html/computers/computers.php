...
{
		include "func/config.php";
		$_SESSION[$session[komp][pagination]]=$_POST;
	  ...
	  $q="select k.id_komp, k.nazwa_smb, i.nazwa, u.symbol, n.symbol, n.nazwa, n.id_abon
				...
				full join
				((pakiety  p join towary_sprzedaz u on p.id_usl=u.id_tows and p.aktywny_od <= '$p[data_od]' and p.aktywny_do >='$p[data_do]') 
				 full join 
				 (taryfy_internet i join taryfy t on i.id_trf=t.id_trf and t.aktywna_od <= '$p[data_od]' and t.aktywna_do >='$p[data_do]') 
...
function AddNewKomp($dbh, $s, $ipc)
{
    ...
    $ID_KONF=$row[0];
		...
		/*
	$q1="select pds.id_pds, pds.adres, pds.maska, pds.brama, pds.broadcast, iv.id_ivn 
	from interfejsy_wezla i1, interfejsy_wezla i2, trakty t, olt o, 
	inst_vlanu iv, przyp_vlanu p, onu onu , adresy_ip a, podsieci pds, miejsca_instalacji m
	where a.id_urz=iv.id_ivn and t.id_ifc2=i2.id_ifc and t.id_ifc1=i1.id_ifc and i1.id_wzl=o.id_olt 
	and  i2.id_wzl=onu.id_onu and p.id_ivn=iv.id_ivn and p.id_ifc=i1.id_ifc and a.id_pds=pds.id_pds 
	and onu.id_msi='$komp[id_msi]' and onu.id_abon='$ID_ABON'";
	$r=Query2($q1, $dbh);
	WyswietlSql($q1);
	if (empty ($r) )
	{
...
and c.id_msi='$komp[id_msi]' and c.id_abon='$ID_ABON'";
		...
		if ( $_POST[ipadres] == 'Prywatny')
			{
				...
				$q4="select pds.id_pds, pds.adres, pds.maska, pds.brama, pds.broadcast, iv.id_ivn from 
				podsieci pds, adresy_ip ai, inst_vlanu iv1, inst_vlanu iv2, budynki b, miejsca_instalacji m where 
				pds.id_pds=ai.id_pds and ai.id_urz=iv.id_ivn and m.id_bud=b.id_bud and b.id_ivn1=iv.id_ivn and m.id_msi='$komp[id_msi]'";
			}
		else
		{
	...
	{*/
		...
		*/
		...
		if ( CheckboxToTable($_POST[ipzewn2]) == 'T' && CheckboxToTable($_POST[ipzewn2_fv]) == 'T' )
			{
				$q="insert into uslugi_dodatkowe values('$ID_KOMP', '$conf[ipzewn2]', 'T', '$DATA_PODL', '$conf[wazne_do]')";
				WyswietlSql($q);
				Query($dbh,$q);	
			}
		else if ( CheckboxToTable($_POST[ipzewn2]) == 'T' && CheckboxToTable($_POST[ipzewn2_fv]) == 'N' )
			{
				$q="insert into uslugi_dodatkowe values('$ID_KOMP', '$conf[ipzewn2]', 'N', '$DATA_PODL', '$conf[wazne_do]')";
				WyswietlSql($q);
				Query($dbh,$q);	
			}
...
/*
		...
		if ( !empty($kmp[id_usl]) && $id_usl!= $kmp[id_usl] && IsNull($id_usl)!='NULL')
		{
		...
		if ( !empty($kmp[id_trf]) && $id_trf!= $kmp[id_trf] && IsNull($id_trf)!='NULL' )
		{
			$q2="update taryfy set id_trf='$id_trf' where id_urz='$ID_KOMP' and aktywna_do='$conf[wazne_do]'";
			WyswietlSql($q2);
			$row=Query($dbh, $q2);
		}
		else if ( empty($kmp[id_trf]) && IsNull($id_trf)!='NULL'  )
		{
			$trf=array(
			'id_urz'=>$ID_KOMP, 'id_trf'=>$id_trf, 'aktywna_od' => $conf[data], 'aktywna_do' => $conf[wazne_do]);
			Insert($dbh, "taryfy", $trf);
		}		
	
			
		$this->AditionalServices($dbh, $ID_KOMP, $kmp, $ipc, $komp[data_podl]);
		...
		{
			$id=explode("_", $k);
			if ( $k!="przycisk" && $id[3] == "usl" && isset($_POST[$k]))
			{
				$q3="delete from pakiety where id_urz='$ID_KOMP' and aktywny_do='$conf[wazne_do]'";
				WyswietlSql($q3);
				Query($dbh, $q3);
		
				$q2="select id_usl, aktywny_od, aktywny_do from pakiety p where id_urz='$ID_KOMP' order by aktywny_do desc limit 1";
	      WyswietlSql($q2);
			  $r=Query2($q2, $dbh);
	      ...
	      WyswietlSql($q1);
			...
			{
				$q3="delete from taryfy where id_urz='$ID_KOMP' and aktywna_do='$conf[wazne_do]'";
				WyswietlSql($q3);
				Query($dbh, $q3);
		
				$q2="select id_trf, aktywna_od, aktywna_do from taryfy p where id_urz='$ID_KOMP' order by aktywna_do desc limit 1";
	      WyswietlSql($q2);
			  $r=Query2($q2, $dbh);

				$q1="update taryfy set aktywna_do='$conf[wazne_do]' where id_trf='$r[0]' and aktywna_od='$r[1]' and aktywna_do='$r[2]'";
	      WyswietlSql($q1);
			  Query($dbh, $q1);
			}
		}
...
function AditionalServices($dbh, $ID_KOMP, $kmp,  $ipc, $aktywna_od=Null)
{
		...
		function Serv($dbh, $kmp, $ID_KOMP, $aktywna_od, $aktywny_do2, $ipc, $srv)
		{
			include "func/config.php";
			...
			//$cb=CheckboxToTable($_POST[$srv_fv]); 
			//echo "$cb <br>" ;
			//$k=$kmp[$srv]; 
			//echo "Kmp $k <br>" ;*/
			
			if ( CheckboxToTable($_POST[$srv]) == 'T' && $kmp[$srv] =='N' )
				{
					if ( CheckboxToTable($_POST[$srv_fv]) == 'T' )
						$q="insert into uslugi_dodatkowe values('$ID_KOMP', '$conf[$srv]', 'T', '$aktywna_od', '$conf[wazne_do]')";
					
					else if ( CheckboxToTable($_POST[$srv_fv]) == 'N' )
						$q="insert into uslugi_dodatkowe values('$ID_KOMP', '$conf[$srv]', 'N', '$aktywna_od', '$conf[wazne_do]')";
			...
			else if ( CheckboxToTable($_POST[$srv]) == 'T' && $kmp[$srv] =='T' )
				{
					if ( CheckboxToTable($_POST[$srv_fv]) == 'T' )
						$q="update uslugi_dodatkowe set fv='T' where id_urz='$ID_KOMP' and id_usl='$conf[$srv]'";
					
					else if ( CheckboxToTable($_POST[$srv_fv]) == 'N' )
						$q="update uslugi_dodatkowe set fv='N' where id_urz='$ID_KOMP' and id_usl='$conf[$srv]'";
						
					WyswietlSql($q);
					Query($dbh,$q);
				}
				
			else if ( CheckboxToTable($_POST[$srv]) == 'N' && $kmp[$srv] =='T' )
			{
			
				$q="update uslugi_dodatkowe set aktywna_do='$aktywny_do2' where id_urz='$ID_KOMP' and id_usl='$conf[$srv]' and aktywna_do='$conf[wazne_do]'";
				WyswietlSql($q);
				Query($dbh,$q);
		...
		Serv($dbh, $kmp,  $ID_KOMP, $aktywna_od, $aktywny_do2, $ipc, 'ipzewn');
		Serv($dbh, $kmp,  $ID_KOMP, $aktywna_od, $aktywny_do2, $ipc, 'ipzewn2');
