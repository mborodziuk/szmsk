 <?php
  ...
  $Q1="select k.id_abon, k.nazwa_smb, p.id_usl, kf.dhcp, kf.ipmac, kf.net,
	...
	// Musi bbyć dodatkowo to zapytanie bo Q1 nie zwraca wyniku dla pakietów, zwraca tylko dla taryf, gdyby zamiast 
	// "on k.id_komp=t.id_urz" było on k.id_komp=p.id_urz to by zwracało dla pakietów a nie dla usług. Nie działa dla 2 naraz.
	$Q4="select id_usl from pakiety where aktywny_od <= '$conf[data]' and aktywny_do >='$conf[data]' and	id_urz='$_GET[komp]'";
	WyswietlSql($Q4);
	$p=Query2($Q4, $dbh);
	...
	$komp=array(
	'id_abon'	=>$row1[0],			'nazwa'	=>	$row1[1],	'id_usl'	=>$p[0],		'dhcp'		=>$row1[3],			
	'ipmac'	=>$row1[4], 			'net'		=>$row1[5],			'proxy'		=>$row1[6],		'dg'	=>$row1[7],			
	'info'=>$row1[8], 				'podlaczony' => $row1[9]	,'fv' => $row1[10], 
	'ipzewn' => 'N', 					'ipzewn_fv' => 'N', 'ipzewn' => 'N', 					'ipzewn_fv' => 'N', 
	'antywirus' => 'N', 'antywirus_fv' => 'N', 'data_podl' => $row1[11],
      ...
      <td>
		...
		<tr>
				...
				if ( !empty ($usl) )
				{
					...
					echo "Kasuj ostatnią usługę";
					echo TableToCheckbox("N", "$l[id_usl]_$l[aktywny_od]_$l[aktywny_do]_usl");
				}
				?>
				</td>
			</tr>
