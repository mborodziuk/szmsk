...
function PayoutList($dbh, $p, &$www)
{
	include "func/config.php";
	$idk=explode(" ", $p[prac]);
	$idp=array_pop($idk);

			$q=" select w.id_wyp, w.data_ksiegowania, w.data_zlecona, w.forma, w.kwota, w.waluta, w.opis, p.nazwa, p.id_prac
    	    from wyplaty w, pracownicy p
          where w.id_kontrah=p.id_prac and w.data_ksiegowania>='$p[data_od]' and w.data_ksiegowania <= '$p[data_do]'";

		if (!empty($p[prac]) && $p[prac] != $conf[select])
			{
				$q.=" and p.id_prac='$idp'";
			}

	if (empty($p[order]) || $p[order] == "Data wpłaty")
			$p[order]="w.data_ksiegowania, w.data_zlecona, w.id_wyp";
	else if ($p[order] == "Nazwa kontrahenta")
			$p[order]="a.nazwa, w.data_ksiegowania, w.data_zlecona, w.id_wyp";
	else if ($p[order] == "Forma wpłaty")
			$p[order]="w.forma, w.data_ksiegowania, w.data_zlecona, w.id_wyp";	
	$q.=" order by $p[order]";
	...
	while ($row =$sth->fetchRow())
	    {
			...
			echo "<td> <a href=\"index.php?panel=fin&menu=payoutupd&wyp=$row[0]\"> $row[0] </a> </td>";
...
{
	$wypl=array 
	(	'id_wyp'		=> $this->FindLastPayment($_POST[data],$dbh),		'data_ksiegowania'	=>	$_POST[data], 		'data_zlecona'	=>	$_POST[data],
		'forma'			=> $_POST[forma], 			'kwota'					=>$_POST[kwota], 	'waluta'			=> "PLN",
		'id_kontrah'	=> FindId2($_POST[id_prac]),					'opis' 			=> $_POST[opis]);

	if ( !empty( $_POST[kwota2] ))
		{
			$wypl[kwota] = $_POST[kwota2];
		}

	Insert($dbh, "wyplaty", $wypl);
}

function Upd($dbh, $id_wyp)
{
	$wyp=array 
	(	'id_wyp'		=> $id_wyp,			'data_ksiegowania'	=>	$_POST[data], 		'data_zlecona'	=>	$_POST[data],
		'forma'			=> $_POST[forma], 'kwota'					=> $_POST[kwota], 	'waluta'			=> "PLN",
		'id_kontrah'	=> FindId2($_POST[id_prac]),		'opis' 			=> $_POST[opis]);

	Update($dbh, "wyplaty", $wyp);
...
{
	$d=explode("-", $date);
	$Q="select id_wyp from wyplaty where data_ksiegowania like '$d[0]-$d[1]-%' and id_wyp like 'WYP/%/$d[1]/$d[0]'order by id_wyp desc limit 1";
	echo $Q;
	$sth=Query($dbh,$Q);
	$row =$sth->fetchRow();
	if (empty($row[0]))
		$wpl="WYP/0001/$d[1]/$d[0]";
	else
						...
						if ( $flag != 0 )
							{
								if ( $flag==1 || $flag==2)
									...
									$Q="select * from wplaty where forma='$forma' and data_ksiegowania like '$y-$m-$d' limit 1";	
								$roww=Query2($Q,$dbh);
							...
							// to ponizej odkomentowac zeby moc wczytac wyciag ktory ma date wsteczna
								//if ( $roww!=null )
								{
										...
										$Q3="select id_wypl from wyplaty where id_wypl like 'WYP/%/$m/$y' order by id_wypl desc limit 1";
										$sth3=Query($dbh,$Q3);
										$row3 =$sth3->fetchRow();
										if ( $row3==null )
											{
