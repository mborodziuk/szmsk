...
//		$this->Image("$conf[pdf_logo1]",15,10,70,15);
	...
	*/
	...
	function MakeFV(&$pdf, $dbh, $dokks, $typ, $adrk=false, $file="../func/config.php")
	{
		...
		$pdf->SetTextColor(128,128,128); 
		$pdf->Text($x1+2,$y2+24,'Nabywca:');
		$pdf->SetTextColor(0,0,0); 

		if ( !empty($dokks[pesel_nip]) && ValidateNip($dokks[pesel_nip]))
		$pdf->Text(60,$y2+24,"NIP: $dokks[pesel_nip]");

		$pdf->SetXY($x1+4,$y2+26);
		$pdf->MultiCell(70,4,"$dokks[nazwa]",0,L,0);
	
		$adres1=$dokks[kod]." ".$dokks[miasto];
		$adres2=$dokks[cecha]." ".$dokks[ulica2]." ".$dokks[ulica]." ".$dokks[nr_bud];
		if (!empty($dokks[nr_mieszk]))
			$adres2.="/$dokks[nr_mieszk]";
						
		$pdf->Text($x1+5,$y2+38,$adres1);	
		$pdf->Text($x1+5,$y2+42,$adres2);	
		...
		if ( $adrk==true && !empty($dokks[k_nazwa]) && !empty($dokks[k_ulica]) && !empty($dokks[k_nr_bud]) && !empty($dokks[k_miasto]) )
				{						
					$dokks[nazwa]=$dokks[k_nazwa];
					$adres1=$dokks[k_kod]." ".$dokks[k_miasto];
					$adres2=$dokks[k_cecha]." ".$dokks[k_ulica]." ".$dokks[k_nr_bud];
					if (!empty($dokks[k_nr_mieszk]))
							$adres2.="/$dokks[k_nr_mieszk]";
				}
		else
				{
					$adres1=$dokks[kod]." ".$dokks[miasto];
		...
		$pdf->SetXY(111,55);
		$pdf->MultiCell(70,4,"$dokks[nazwa]",0,L,0);
		$pdf->Text(112,71,$adres1);	
		$pdf->Text(112,75,$adres2);	
		...
		while ( $row1=$sth1->fetchRow() )
		{
			...
			$pdf->Cell(6,$poz_h+$poz_h2,"$Lp.",1,0,'C');
			
			if ( strlen($row1[0]) > 70 )
			{
				$pdf->Cell(76,$poz_h+$poz_h2,"",1,0,'L');
				$pdf->SetXY(21,100+$poz_h2+($n-1)*$poz_h);
				$pdf->MultiCell(70,4,"$row1[0]",0,L,0);
				$pdf->SetXY(97,92+$poz_h2+($n-1)*$poz_h);		
			}
			else
			    $pdf->Cell(76,$poz_h, $row1[0],1,0,'L');
		
			$pdf->Cell(12,$poz_h+$poz_h2, $row1[1],1,0,'C');
			$pdf->Cell( 7,$poz_h+$poz_h2, $row1[2],1,0,'C');
			$pdf->Cell( 7,$poz_h+$poz_h2, $row1[3],1,0,'C');
			...
			$pdf->Cell(15,$poz_h+$poz_h2, number_format($jedn_netto, 2, ',',''),1,0,'R'); //Cena jedn. netto
			$pdf->Cell(18,$poz_h+$poz_h2, number_format($netto, 2, ',','') ,1,0,'R'); // Netto
			$pdf->Cell(7,$poz_h+$poz_h2, $row1[5],1,0,'C');	// % VAT
			if ($row1[5]=="-")
			{
				$pdf->Cell(15,$poz_h+$poz_h2,"-",1,0,'C');
			}
			else 
				$pdf->Cell(15,$poz_h+$poz_h2,number_format(round($kwota_vat,2), 2,',',''),1,0,'R');
			
			$pdf->Cell( 18, $poz_h+$poz_h2, number_format(round($brutto ,2), 2, ',',''), 1, 0, 'R' );
		...
		$dokks[saldo] = round( $dokks[saldo], 2);
		$dokks[saldo] = number_format(-$dokks[saldo], 2, ',','');
			

		
		$pdf->SetXY(75,139+$poz_h*$n+$poz_h2);
		$pdf->SetTextColor(128,128,128); 
		$pdf->Cell(72,10,"Zobowiązanie po wystawieniu bieżącej faktury :",0,0,'R');
		$pdf->SetTextColor(0,0,0); 
		$pdf->SetFont('Tahoma','B',9);
		$pdf->Cell(45,10,"$dokks[saldo] zł",0,1,'L');
		$pdf->SetFont('Tahoma','',8);
		
		$pdf->SetXY(75,144+$poz_h*$n+$poz_h2);
		$pdf->SetTextColor(128,128,128); 
		$pdf->Cell(72,10,"Fakturę wystawił(a) :",0,0,'R');
		$pdf->SetTextColor(0,0,0); 
		$pdf->Cell(45,10,"$dokks[wystawil]",0,1,'L');
		
		$pdf->SetXY(75,149+$poz_h*$n+$poz_h2);
		$pdf->SetTextColor(128,128,128); 
		$pdf->Cell(72,10,"Jako tytuł przelewu prosimy podać :",0,0,'R');
		$pdf->SetTextColor(0,0,0); 
		$pdf->Cell(60,10,"$dokks[id_abon] za FV nr $dokks[nr_d]",0,0,'L');
		$pdf->SetFont('Tahoma','B',6);

		$pdf->Image('/home/szmsk/public_html/graphics/krdsmall.jpg',18,143+$poz_h*$n+$poz_h2,15);
		$pdf->SetXY(30,143+$poz_h*$n+$poz_h2);
		...
		if (!empty($dokks[uwagi]))
		{
			$pdf->SetTextColor(0,0,0);
			$pdf->SetFont('Tahoma','',8);
			$pdf->SetXY($x1, 169+$poz_h*$n+$poz_h2);
			//$pdf->Text($x1,169+$poz_h*$n+$poz_h2, "UWAGI:  $dokks[uwagi]");
			$pdf->MultiCell(200,4,"UWAGI:  $dokks[uwagi]" ,0,'L',0);
		}
		return($pdf);
	}
...
k.nazwa, k.ulica, k.nr_bud, k.nr_mieszk, k.kod, k.miasto, kw.numer, a.uwagi, u.cecha, k.cecha, u.nazwa2, a.saldo
from
		...
		$dokks=array
			(
				...
				'k_nazwa' 		=> $row[15],	'k_ulica' 			=> $row[16],'k_nr_bud'		=> $row[17], 'k_nr_mieszk' => $row[18],
					...
					'cecha'			=> $row[23],	'k_cecha'				=> $row[24], 'ulica2'			=> $row[25], 'saldo'			 => $row[26]
				);
...
and  o.nr_nob='$nr'";	
	 ...
	 if ( !empty($ZALICZKI) )
		...
		while ($row=$sth->fetchRow() )
		{
			...
			//	$pdf->SetFillColor(5,5,5);
				...
				$pdf->Cell($WSN_W, $W2,'',1,0,'C');
				$pdf->Cell($PV_W,  $W2,'',1,0,'C');
				$pdf->Cell($WSN_B, $W2,'',1,0,'C');		
				$pdf->Cell($WSN_BOO, $W2,'',1,0,'C');		
				...
				$pdf->SetXY($ML+$LP_W+$NRD_W+$MW_W+$DW_W+$TP_W+$N_W+$NIP_W+$WSN_W+$PV_W+$WSN_B+$PV_W23+$WSN_W23-1+$WSN_BOO, $y);
				$pdf->MultiCell($WSN_B,3,'Wartość sprzedaży brutto [w zł] Odwr. obc.',0,C,0);
			}
	...
	//		$razem_b+=$razem_brutt
				...
				$r_razem_brutto_oo=round($razem_brutto_oo, 2);
				$razem_b_oo+=$r_razem_brutto_oo;
			...
			$pdf->Cell($WSN_B,$W, number_format($r_razem_brutto, 2,',',''),1,0,'C');

			$pdf->Cell($WSN_BOO,$W, number_format($r_razem_brutto_oo, 2,',',''),1,0,'C');
		...
		$pdf->Cell($LP_W+$NRD_W+$MW_W+$DW_W+$TP_W+$N_W+$NIP_W, $W, "RAZEM : ", 1, 0, 'R');
					...
					$pdf->Cell($WSN_B,$W, number_format($razem_b, 2,',',''),1,0,'C');	
					$pdf->Cell($WSN_BOO,$W, number_format($razem_b_oo, 2,',',''),1,0,'C');	
		return($pdf);
	}
 ...
 and s.wazne_od <= '$conf[data]' and s.wazne_do >='$conf[data]'
	...
	function PonaglenieZaplaty(&$pdf, $dbh, $id_spw, $file="../func/config.php")
	{
		...
		if ( !empty($dluznik[k_nazwa]) && !empty($dluznik[k_ulica]) && !empty($dluznik[k_nr_bud]) && !empty($dluznik[k_miasto])  )
			...
			$tekst1=" ";
			$tekst2="Z przykrością stwierdzamy, że konto Państwa wskazuje zaległe należności za usługi na łączną kwotę (wraz z bieżącą fakturą):";
			$tekst3="$dluznik[saldo] zł brutto."; 
			$tekst4=" ";
			$tekst5="Należności te wynikają z braku zapłaty za usługi dostępu do Internetu, usługi telewizyjne, telekomunikacyjne lub inne usługi multimedialne.";
			$tekst6="";
			$tekst7="Uprzejmie prosimy o niezwłoczne wpłacenie w/w kwoty na rachunek bankowy:";
			$tekst8="NETICO S.C.  - $dluznik[konto]";
			$tekst9="";
			$tekst10="Jeżeli płatność została już dokonana a nie jest jeszcze zaksięgowana na naszym koncie to prosimy potraktować niniejsze pismo za niebyłe.";
			$tekst11="";		
			$tekst12="Jeśli natomiast, z powodu braku czasu, mają Państwo problemy z terminowym regulowaniem należności za wystawione przez NETICO S.C. faktury za usługi zachęcamy do korzystania z bezgotówkowych form płatności - stałe zlecenie bankowe lub przelew internetowy.";
			$tekst13="";		
			$tekst14="W razie jakichkolwiek wątpliwości dotyczących zaległych płatności prosimy o kontakt pod adresem e-mail: finanse@netico.pl lub numerem telefonu: 32 745 33 33.";	
			$tekst15="";	
...
from 
(((abonenci a left join maile e on a.id_abon=e.id_podm) join nazwy n on a.id_abon=n.id_abon and n.wazne_od <= '$um[data_zycie]' and n.wazne_do >= '$um[data_zycie]')  
left join
(telefony t left join (kontakty k left join telefony tl on k.id_podm=tl.id_podm)  on k.id_podm=t.id_podm and k.wazne_od <= '$um[data_zycie]' and k.wazne_do >='$um[data_zycie]')
on t.id_podm=a.id_abon)
join
((ulice  u join budynki b on u.id_ul=b.id_ul ) join  (adresy_siedzib s join konta_wirtualne w on s.id_abon=w.id_abon) on b.id_bud=s.id_bud and s.wazne_od <= '$um[data_zycie]' and s.wazne_do >= '$um[data_zycie]') on a.id_abon=s.id_abon where 
...
/*
	...
	*/	
	...
	on p.id_urz=k.id_komp and k.id_abon='$um[id_abon]' and k.fv='T' and k.podlaczony='T'";
	//		WyswietlSql($q4);
	$kmp=array(	);
	$sth4=Query($dbh,$q4);
	while ($row4=$sth4->fetchRow())
	{
		$q6="select fv from uslugi_dodatkowe where (id_usl='$conf[ipzewn]' or id_usl='$conf[ipzewn2]')   and aktywna_od<= '$um[data_zycie]' and aktywna_do>= '$um[data_zycie]' and id_urz='$row4[0]'";
...
/*
	...
	*/
	...
	$q20="select r.id_rtr, r.typ, r.mac, r.data_aktywacji, t.nazwa, t.symbol, p.aktywny_od
				from 
				(router r join belong b on b.id_urz=r.id_rtr and b.nalezy_od <= '$um[data_zycie]' and b.nalezy_do >='$um[data_zycie]')
				join 
				(towary_sprzedaz t join pakiety p on p.id_usl=t.id_tows and p.aktywny_od <= '$um[data_zycie]' and p.aktywny_do >='$um[data_zycie]')
	on p.id_urz=r.id_rtr and b.id_abon='$um[id_abon]'";
	...
	{		
		...
		$r=array(
		'id_rtr' 	=> $row20[0], 	  'typ'		  => $row20[1],    'mac'							=> $row20[2],     'aktywny_od'							=> $row20[8], 
		'data_aktywacji' 	=>$row20[3], 	'nazwa' 					=> $row12[4],'symbol' 					=> $row12[5]
		);
		array_push($rtr, $r);		
	...
	$tresc1=Inf($abon, $um, $kmp, $tlv, $stb, $srv, $rtr );
	$tresc2=Deklaracja($abon, $um);
	$tresc3=Weksel($abon, $um);
  //define("_MPDF_PATH", '/usr/share/php/mpdf/');
