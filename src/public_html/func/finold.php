	...
	foreach ($_POST as $k => $v)
		{
			if ( $k!="przycisk" && isset($_POST[$k]))
			{
				...
				$plik1=explode("/", $plik[0]);
				$forma=array_pop($plik1);
			  ...
			  if ( $flag == 1 || $flag == 2)
			    {
						$Q="select * from wplaty where id_wpl like 'WPL/%/$str1/$str2' and forma='$forma' limit 1";
						$sth=Query($dbh,$Q);
						$roww =$sth->fetchRow();
						if ( $roww==null )
							{
								$Q2="select id_wpl from wplaty where id_wpl like 'WPL/%/$str1/$str2' order by id_wpl desc limit 1";
								$sth2=Query($dbh,$Q2);
								$row2 =$sth2->fetchRow();
								if ( $row2==null )
									{                                                                                            
										$id_wpl="WPL/0000/$str1/$str2";
										//echo "$id_wpl";
									}
										//echo "$k";
								else 
								...
								$Q3="select id_wypl from wyplaty where id_wypl like 'WYP/%/$str1/$str2' order by id_wypl desc limit 1";
								$sth3=Query($dbh,$Q3);
								$row3 =$sth3->fetchRow();
								if ( $row3==null )
									{
										$id_wyp="WYP/0000/$str1/$str2";
												 //echo "$id_wpl";
									}
									 //echo "$k";
								else
